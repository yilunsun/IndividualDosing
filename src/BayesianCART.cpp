...
List BayesianCART(NumericMatrix x, // baseline covariate
                  ...
                  String prior_leaf = "uniform", int MinimumLeafSize=1, unsigned int seed=123, int MinLeaf=30, bool verbose=true, bool np=false, bool keepbest=true
                  )
{
  Random ran(seed);
  ...
  for (int l = 0; l < nChain; l++) 
  {
    ...
    for (int i = 0; i < Length; i++)
    {
      ...
      tree = mcmc.Iterate(tree,proposal1,every,nAcc,ran,T0,proprob,complexity,verbose,np,keepbest);
